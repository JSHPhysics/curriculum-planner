...
## DEC-045 — Folder export is a NESTED FOLDER TREE mirroring the curriculum hierarchy, not one xlsx per HT/topic
**Date:** 2026-05-17
**Session:** 27
**Status:** Accepted, supersedes the v1.1 folder-export design in DEC-039

### Context
v1.1 (DEC-039) shipped "folder export" as: one .xlsx per visible half-term (or per topic), zipped into a folder. I built it from a wrong reading of the user's request — they wanted something else entirely.
...
So:
- **The xlsx export is ONE file for the whole subject** (which the existing `exportSubjectToXlsx` already did — five sheets — unchanged).
- **The folder export builds a NESTED TREE** of folders mirroring `subject → HT or topic → sub-topic → lesson`. Teachers drop worksheets, slides, video links into pre-built slots.

User-confirmed design choices (via AskUserQuestion):
1. **Tree depth: down to lesson, with per-lesson info file** — every leaf lesson folder contains `_lesson-info.txt` describing the slot (title, week, sub-topic, topic, objectives, practical, depth/separate flags).
2. **Top-level structure: user picks (radio)** — by half-term OR by topic.
3. **Folder naming: just the human-readable name** ("Key concepts of physics", not "T1 · Key concepts" or "01 Key concepts").

### Decision

**(1) New `exportFolderStructure(subject, rootBy, options?)` in `src/model/folderExport.ts`.**
- Returns `{ suggestedRootName, entries }`. Each entry has `path` (forward-slash relative path) and optional `content` (Uint8Array; absent = folder marker, present = a file).
- `rootBy = "half-term"`: `<HT>/<topic>/<sub-topic>/<lesson title>/_lesson-info.txt`. Every visible HT gets a folder even when empty (pre-built future-planning slots).
- `rootBy = "topic"`: `<topic>/<sub-topic>/<lesson title>/_lesson-info.txt`. Topics emit in spec order; sub-topics in spec order; lessons in calendar order.
- Custom blocks (test/lesson/unit/assessment/retrieval/other) appear as leaf folders too — by-HT mode puts them under their HT folder; by-topic mode collects them under an "Other blocks" folder at the root.

**(2) `_lesson-info.txt` content** — UTF-8 text file containing:
- Lesson title, lesson number, topic code+name, sub-topic code+name
- Half-term (year + label), dates if known
- Practical if any, depth/separate flags
- Bulleted objectives (depth-tagged objectives prefixed with `[depth]`)
- Friendly closing line: "Drop your resources for this lesson into this folder. Generated by Curriculum Planner; safe to delete this file."

**(3) `packTreeAsZip(tree)`** packs the tree into a single .zip via JSZip, preserving folder structure (empty directory entries for folder markers, file entries for content).

**(4) IPC: `file:saveFolderXlsx` renamed to `file:saveFolderTree`.**
- New shape: `{ suggestedRootName, entries: [{ path, content? }] }`.
- Two-pass write: folder markers first (mkdir -p), then file content.
- `path` segments are split on `/` and joined with the OS separator.

**(5) UI: `ExportModal` rewritten** for the new two-mode shape:
- Two top-level radios: "Single workbook" / "Folder structure".
- When "Folder structure" is picked, sub-options appear:
  - Top-level grouping radio: "By half-term" / "By topic"
  - Output radio: "Zip file" / "Folder on disk" (zip is default — convenient for sharing)
...
**(6) Naming convention** (per user choice):
- Half-terms: `"Y9 Aut 1"` — happens to sort alphabetically into chronological order because "Aut" < "Spr" < "Sum".
- Topics: `"Key concepts of physics"` — sub-topics within sort alphabetically. Loses the T1/T1a code visibility for the trade of cleaner names; user explicitly chose this.
- Sub-topics: `"Units and measurement"`.
- Lessons: `"SI units and prefixes"` (lesson title; falls back to `"Lesson N"` if title missing).
- Path-safety scrubbing: `[\\/:*?"<>|]` replaced with space, whitespace collapsed, trailing dots stripped.

**(7) Honour existing filters:**
- `subject.config.hiddenYears` — no folder for hidden HTs (by-HT) or for placements in hidden years (by-topic).
- `subject.config.includeDepth` — depth lessons drop out via existing `effectiveLessonsInPlacement`. Sub-topics with no surviving lessons emit no lesson folders.

### Alternatives considered

- **"All the way to per-lesson folders"** (no info file). Simpler. User picked the with-info-file option because the empty folders would lose context — teachers wouldn't remember which lesson goes where without opening the planner.

- **"Down to sub-topic folders"** (no lesson folders). Faster to set up but loses the per-lesson resource granularity that teachers actually want.

- **"Always by half-term"** / **"Always by topic"** (no user choice). Simpler menu. Rejected per user: both structures have legitimate use cases (term-by-term planning vs cross-year resource reuse).

- **"Code-prefix + name"** ("T1a - Kinematics") or **"Number-prefix"** ("01 Key concepts"). Both preserve sort order regardless of name alphabet. User picked "just human-readable name" — clean visually, accepts that within-topic sub-topic order will sort alphabetically rather than by curriculum order.

### Consequences

- **v1.2 folder export is unreleasable as is** — completely different feature shape from v1.1's "one xlsx per HT/topic". Anyone who used the v1.2 folder export should re-export with v1.2.1.
- **The old `exportByHalfTermFolder` / `exportByTopicFolder` / `packBundleAsZip` are gone.** No deprecation shim — they were just shipped in v1.2.0 (a few hours ago) and the design was wrong.
- **The IPC name changed** — `saveFolderOfXlsx` → `saveFolderTree`. Tests and the mock fixture updated together.
- **Hidden-year handling for empty HTs**: even empty visible HTs get a folder (a planning slot). Hidden HTs get nothing. This is intentional — the teacher should see "Y10 Spr 1 is empty, I need to plan it" not "I can't tell if I've planned Y10 Spr 1 or not".
- **`_lesson-info.txt`** is written even for empty cells' future-proofed folders... actually no: only LEAF lesson folders get the info file. Empty HTs are just empty folders.
- **Per-topic sub-topic sort order**: alphabetical, NOT spec order. User accepted this trade-off for cleaner names. If it becomes a pain point, swap to "Code-prefix" naming in one line.

### Related
- `SPEC.md` §6 (Export) — will fold the new folder-tree shape into §6 at next consolidation
- `src/model/folderExport.ts` (`exportFolderStructure`, `packTreeAsZip`, `FolderTreeEntry`, `FolderTreeExport`, `FolderRootBy`)
- `src/components/ExportModal.tsx` (two-mode + sub-radio shape)
- `src/App.tsx` (`handleExportConfirm` dispatch)
- `electron/main.ts` + `electron/preload.ts` + `src/types/api.d.ts` (`saveFolderTree` IPC)
- `tests/e2e/fixtures.ts` (mock `saveFolderTree`)
- [DEC-039](#dec-039) — superseded (the "one xlsx per HT/topic" design)
- [DEC-040](#dec-040) (depth filtering — applied here too), [DEC-036](#dec-036) (hidden years — applied here too), [DEC-044](#dec-044) (CustomBlock categories — leaf folders for non-spec content)
